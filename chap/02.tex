...
\marginpar{\footnotesize 使阶码值为 $1 - Bias$ 而不是简单的 $-Bias$ 似乎是违反直觉的，但这种方式提供了一种从非规格化值平滑转换到规格化值的方法}

非规格化值由两个用途。一是提供了一种表示数值 0 的方法（包括 $-0.0$ 和 $+0.0$），二是表示那些非常接近于 $0.0$ 的数。
...
由于舍入造成的精度差异，浮点运算满足交换性和单调性，但是不满足结合性和分配性。
